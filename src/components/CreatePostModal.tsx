...
type CreatePostModalProps = { isOpen: boolean; onClose: () => void; onPost?: (post: { title: string; description: string; tag: string; imageUrl?: string }) => void }
const TAGS = ['Atölye', 'Etkinlik', 'Turnuva', 'Duyuru', 'Haber', 'Fotoğraf']
const CLUBS = ['Developer Student Community', 'ESports Society', 'Müzik & Sahne Sanatları', 'Girişimcilik Kulübü', 'Fotoğrafçılık Topluluğu', 'Bilim & Araştırma Kulübü']
...
export const CreatePostModal = ({ isOpen, onClose, onPost }: CreatePostModalProps) => {
  ...
  const reset = () => { setTitle(''); setDescription(''); setSelectedTag('Etkinlik'); setImagePreview(null); if (fileRef.current) fileRef.current.value = '' }
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault(); if (!title.trim()) return
    onPost?.({ title: title.trim(), description: description.trim(), tag: selectedTag, imageUrl: imagePreview ?? undefined })
    reset(); onClose()
  }
    ...
    <Modal isOpen={isOpen} onClose={onClose}>
      <div className="w-full max-w-lg overflow-hidden rounded-2xl shadow-2xl"
        ...
        <div className="bg-gradient-to-r from-violet-600 to-indigo-600 px-6 py-4 flex items-center justify-between">
          ...
          <button type="button" onClick={() => { reset(); onClose() }} className="rounded-lg p-1.5 text-white/60 hover:bg-white/15 hover:text-white transition-all">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          <div>
            ...
            <select className="input-pro w-full" style={{ WebkitAppearance: 'none' }}>{CLUBS.map(c => <option key={c}>{c}</option>)}</select>
          </div>
          <div>
            <label className="block text-xs font-semibold mb-1.5" style={{ color: 'var(--text3)' }}>Başlık *</label>
            <input type="text" required value={title} onChange={e => setTitle(e.target.value)} placeholder="Gönderi başlığını yaz..." className="input-pro w-full" />
          </div>
          <div>
            <label className="block text-xs font-semibold mb-1.5" style={{ color: 'var(--text3)' }}>Açıklama</label>
            <textarea rows={3} value={description} onChange={e => setDescription(e.target.value)} placeholder="Kısa açıklama..." className="input-pro w-full resize-none" />
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {TAGS.map(tag => (
                <button key={tag} type="button" onClick={() => setSelectedTag(tag)}
                  className={['rounded-full px-3 py-1 text-xs font-semibold transition-all', selectedTag === tag ? 'bg-violet-600 text-white' : 'hover:text-violet-500'].join(' ')}
                  style={selectedTag !== tag ? { background: 'var(--input-bg)', border: '1px solid var(--bc2)', color: 'var(--text2)' } : undefined}>
                  {tag}
                </button>
              ))}
            </div>
          ...
          <div>
            {imagePreview ? (
              <div className="relative">
                <div className="aspect-[16/9] overflow-hidden rounded-xl"><img src={imagePreview} alt="" className="h-full w-full object-cover" /></div>
                <button type="button" onClick={() => { setImagePreview(null); if (fileRef.current) fileRef.current.value = '' }} className="absolute right-2 top-2 flex h-7 w-7 items-center justify-center rounded-full bg-black/60 text-white">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="h-3.5 w-3.5"><path d="M6.28 5.22a.75.75 0 0 0-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 1 0 1.06 1.06L10 11.06l3.72 3.72a.75.75 0 1 0 1.06-1.06L11.06 10l3.72-3.72a.75.75 0 0 0-1.06-1.06L10 8.94 6.28 5.22Z"/></svg>
                </button>
              </div>
            ) : (
