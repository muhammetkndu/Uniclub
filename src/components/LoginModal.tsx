      ...
      <div className="w-full max-w-sm overflow-hidden rounded-2xl shadow-2xl"
        ...
        <div className="px-7 py-6">
          <form className="space-y-4" onSubmit={e => e.preventDefault()}>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <button type="button" className="text-[11px] font-semibold text-violet-500 hover:text-violet-400">Şifremi unuttum</button>
              </div>
              <div className="relative">
                <input type={showPassword ? 'text' : 'password'} required placeholder="••••••••" className="input-pro w-full pr-10" />
                <button type="button" onClick={() => setShowPassword(s => !s)} className="absolute inset-y-0 right-3 flex items-center" style={{ color: 'var(--text3)' }} tabIndex={-1}>
            ...
            <button type="submit" className="btn-primary w-full py-2.5 text-sm justify-center">Giriş Yap</button>
          </form>
          ...
          <p className="mt-4 text-center text-xs" style={{ color: 'var(--text3)' }}>
            Hesabın yok mu? <button type="button" onClick={onClose} className="font-bold text-violet-500 hover:text-violet-400">Kayıt ol →</button>
          </p>
        </div>
      </div>
    </Modal>
