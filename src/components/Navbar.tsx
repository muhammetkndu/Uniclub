...
/* NavLink style helper — active state via CSS tokens */
      ...
      <nav className="border-b" style={{ background: 'var(--nav-bg)', backdropFilter: 'blur(16px)', borderColor: 'var(--bc)', transition: 'background 0.3s' }}>
        <div className="mx-auto flex max-w-7xl items-center gap-5 px-4 py-3.5 lg:px-8">
          ...
          <Link to="/" className="flex shrink-0 items-center gap-2.5 group">
            ...
            <span className="text-sm font-bold tracking-tight transition-colors group-hover:text-[var(--accent-text)]"
              style={{ color: isDark ? 'var(--text1)' : 'var(--text1)' }}>
              UniClub
            </span>
          </Link>
          ...
          <form onSubmit={handleSearch} className="relative flex flex-1 items-center rounded-xl transition-all duration-200"
            ...
            {search && (
              <button aria-label="Ara" type="submit" className="mr-2 rounded-lg px-2.5 py-1 text-xs font-semibold text-white transition-colors"
                style={{ background: 'var(--accent)' }}>Ara</button>
            )}
          </form>
          ...
          <div className="hidden items-center gap-2 md:flex shrink-0">
            ...
            <button
              ...
              style={{
                ...
                color: isDark ? 'var(--warning)' : 'var(--accent)',
              }}>
          ...
          {/* Mobile hamburger */}
          <button aria-label="Menüyü aç/kapat" type="button" onClick={() => setMenuOpen(o => !o)}
          ...
          <div className="border-t md:hidden" style={{ background: 'var(--bg)', borderColor: 'var(--bc)' }}>
            <div className="flex flex-col gap-0.5 px-4 py-3">
              ...
              <div className="mt-2 flex items-center gap-2 border-t pt-3" style={{ borderColor: 'var(--bc)' }}>
                <button aria-label={isDark ? 'Aydınlık moda geç' : 'Karanlık moda geç'} type="button" onClick={toggle}
                  className="flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-semibold transition-all"
                  style={{ border: '1px solid var(--bc2)', color: isDark ? 'var(--warning)' : 'var(--accent)', background: 'var(--input-bg)' }}>
                  {isDark ? <><SunIcon /> Aydınlık</> : <><MoonIcon /> Karanlık</>}
                </button>
                <button aria-label="Giriş Yap" type="button" onClick={() => { setMenuOpen(false); onLoginClick() }}
                  className="flex-1 rounded-xl py-2 text-sm font-medium btn-cancel">Giriş Yap</button>
                <button aria-label="Kayıt Ol" type="button" onClick={() => { setMenuOpen(false); onSignupClick() }}
                  className="flex-1 btn-primary text-sm py-2 justify-center">Kayıt Ol</button>
              </div>
            </div>
          </div>
        )}
